...
def test_select_z():
    ...
    selected = dataset.select_z(z_val - 0.1, z_val + 0.1)
...
def test_select_k():
    ...
    kmin = k_val - 0.1
    kmax = k_val + 0.1
    selected = dataset.select_k(kmin, kmax)
    assert isinstance(selected, DataSet)
